...
import org.egov.tlcalculator.repository.builder.AccHeadQueryBuilder;
...
import org.egov.tlcalculator.repository.AccHeadRepository;
...
public class CalculationService {
    ...
    @Autowired
    private AccHeadRepository headrepository;

    @Autowired
    private BillingslabQueryBuilder queryBuilder;

    @Autowired
    private AccHeadQueryBuilder queryBuilderAccHead;
    ...
    private Calculation getTaxHead(TradeLicense license, RequestInfo requestInfo, Object mdmsData){
        Calculation calculation = new Calculation();
        ...
        // String datefrom = jdf.format(taxPeriodFrom);

        // String dateto = jdf.format(taxPeriodTo);

        // System.out.println("datefrom"+datefrom);
        // System.out.println("dateto"+dateto);
        // System.out.println("CommencementDate"+jdf.format(license.getCommencementDate()));


        if(license.getCommencementDate().compareTo(taxPeriodTo)>0)
            calculation.setTaxType(TLCalculatorConstants.TRADE_ADV);
        else if(license.getCommencementDate().compareTo(taxPeriodFrom)<0)
            calculation.setTaxType(TLCalculatorConstants.TRADE_ARR);
        else if(license.getCommencementDate().compareTo(taxPeriodFrom)>=0)
            calculation.setTaxType(TLCalculatorConstants.TRADE_CUR);
    ...
    private EstimatesAndSlabs getBaseTax(CalulationCriteria calulationCriteria, RequestInfo requestInfo, Object mdmsData) {
        ...
        searchCriteria.setEnterprisetype(license.getTradeLicenseDetail().getEnterpriseType().toString());
        ...
        Calculation calculation=getTaxHead(license, requestInfo, mdmsData);
        ...
        // getAccessoryFeeAndBillingSlabIds(license,CalculationType
        ...
        EstimatesAndSlabs estimateandslabs=getTaxHeadEstimate(license,calculation,totalTax);
        ...
        //     estimateList.add(estimate);
        // }
        // estimate.setTaxHeadCode(estimate.getTaxHeadCode());
        // estimateList.add(estimate);
        // estimatesAndSlabs.setEstimates(estimateList);

        return estimateandslabs;
    }
    ...
    private FeeAndBillingSlabIds getTradeUnitFeeAndBillingSlabIds(TradeLicense license, CalculationType calculationType) {
        ...
        for (TradeUnit tradeUnit : tradeUnits) {
            if (tradeUnit.getActive()) {
                ...
                // }
                searchCriteria.setEnterprisetype(license.getTradeLicenseDetail().getEnterpriseType());
                // Call the Search
     ...
     * @return TradeUnit Fee and billingSlab used to calculate it
     */
    private EstimatesAndSlabs getTaxHeadEstimate(TradeLicense license, Calculation calculation,BigDecimal totTax) {
        EstimatesAndSlabs estimatesandslabs=new EstimatesAndSlabs();
        List<TaxHeadEstimate> estimateList = new ArrayList<>();
        List<Object> preparedStmtList = new ArrayList<>();
        CalculationSearchCriteria searchCriteria = new CalculationSearchCriteria();
        searchCriteria.setTaxType(calculation.getTaxType());
        // Call the Search
        String query = queryBuilderAccHead.getSearchQuery(searchCriteria, preparedStmtList);
        List<TaxHeadEstimate> taxHeadEstimates = headrepository.getDataFromDB(query, preparedStmtList);
        for (TaxHeadEstimate taxHeadEstimate : taxHeadEstimates) {
            if(taxHeadEstimate.getRate().compareTo(BigDecimal.ZERO) == 0)
                taxHeadEstimate.setEstimateAmount(totTax);
            else
                taxHeadEstimate.setEstimateAmount(taxHeadEstimate.getRate());
            taxHeadEstimate.setCategory(Category.TAX);
            estimateList.add(taxHeadEstimate);
        }
        estimatesandslabs.setEstimates(estimateList);
        if (CollectionUtils.isEmpty(estimatesandslabs.getEstimates()))
            throw new CustomException("TaxHead ERROR", "No Head Found for the given Tax Type");
